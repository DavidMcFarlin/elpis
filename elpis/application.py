...
class ElpisApplication(Gtk.Application):
    ...
    def __init__(self, version=''):
        super().__init__(application_id='io.github.DavidMcFarlin.Elpis',
        ...
        os.environ['PULSE_PROP_application.name'] = 'Elpis'
        os.environ['PULSE_PROP_application.id'] = 'io.github.DavidMcFarlin.Elpis'
        os.environ['PULSE_PROP_application.version'] = version
        os.environ['PULSE_PROP_application.icon_name'] = 'io.github.DavidMcFarlin.Elpis'
    ...
    def do_command_line(self, command_line):
        ...
        # Show the Elpis log since last reboot and exit
        if options.contains('last-logs'):
            ...
            self._print(command_line, 'Elpis {}'.format(self.version))

            reader = journal.Reader()
            reader.this_boot()
            reader.add_match(SYSLOG_IDENTIFIER='io.github.DavidMcFarlin.Elpis')
            ...
            if not got_logs:
                self._print(command_line, _('No logs for Elpis present for this boot.'))
    ...
    def do_activate(self):
        if not self.window:
            logging.info('Elpis {}'.format(self.version))
            self.window = ElpisWindow(self, self.test_mode)
    ...
    def help_cb(self, action, param):
        open_browser("https://github.com/DavidMcFarlin/elpis/wiki", self.window)
