...
class MprisPlugin(ElpisPlugin):
    ...
    def on_prepare(self):
        if self.bus is None:
            ...
            try:
                self.mpris = ElpisMprisService(self.window, connection=self.bus)
...
class ElpisMprisService(DBusServiceObject):
    ...
    MEDIA_PLAYER2_RATINGS_IFACE = 'org.mpris.MediaPlayer2.ExtensionElpisRatings'

    TRACK_OBJ_PATH = '/io/github/DavidMcFarlin/Elpis/TrackId/'
    NO_TRACK_OBJ_PATH = '/org/mpris/MediaPlayer2/TrackList/NoTrack'
    PLAYLIST_OBJ_PATH = '/io/github/DavidMcFarlin/Elpis/PlaylistId/'
    ...
    def __init__(self, window, **kwargs):
        '''Creates a ElpisMprisService object.'''
    ...
    def connect(self):
        '''Takes ownership of the Elpis mpris Interfaces.'''
        self._reset()
        ...
        self.bus_id = Gio.bus_own_name_on_connection(
            self.connection,
            'org.mpris.MediaPlayer2.io.github.DavidMcFarlin.Elpis',
    ...
    def disconnect(self):
        '''Disowns the Elpis mpris Interfaces.'''
    ...
    def _get_metadata(self, window, song):
        ...
        # Map elpis ratings to something MPRIS understands
        userRating = 1.0 if song.rating == 'love' else 0.0
        duration = song.get_duration_sec() * 1000000
        elpis_rating = window.song_icon(song) or ''
        trackid = self._track_id_from_song(song)
        ...
        metadata = {
            ...
            'elpis:rating': GLib.Variant('s', elpis_rating),
        }
    ...
    def DesktopEntry(self):
        ...
        return 'io.github.DavidMcFarlin.Elpis'
    ...
    def HasElpisExtension(self):
        '''b Read only Interface MediaPlayer2.ExtensionElpisRatings'''
        # This property exists so that applets can check it to make sure
        # the MediaPlayer2.ExtensionElpisRatings interface actually exists.
        # It's much more convenient for them then wrapping all their
        # ratings code in the equivalent of a try except block.
        # Not all versions of Elpis will have this interface.
    ...
    def LoveSong(self, TrackId):
        '''(o) -> nothing Interface MediaPlayer2.ExtensionElpisRatings'''
        song = self._song_from_track_id(TrackId)
        if song:
...
class MprisPluginPrefsDialog(Gtk.Dialog):
    ...
    def __init__(self, window, settings):
        ...
        box = Gtk.Box()
        label = Gtk.Label()
        label.set_markup('<b>{}</b>\n{}'.format(_('Hide Elpis on Close'), _('Instead of Quitting')))
    ...
    def _disable_hide_on_delete(self):
        if self.delete_handler:
            self.elpis.disconnect(self.delete_handler)
        self.delete_handler = self.elpis.connect('close-request', self.elpis.on_destroy)

    def _enable_hide_on_delete(self):
        if self.delete_handler:
            self.elpis.disconnect(self.delete_handler)
        self.delete_handler = self.elpis.connect('close-request', self.on_close)
