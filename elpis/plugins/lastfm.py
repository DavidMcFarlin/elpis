...
from elpis.gobject_worker import GObjectWorker
from elpis.plugin import ElpisPlugin
from elpis.util import open_browser
...
class LastfmPlugin(ElpisPlugin):
    ...
    def _dialog(self, username):
        ...
        if self.enabled:
            ...
            secondary_text = _('But Elpis is still authorized with this Last.fm account:')
...
class LastFmAuth(Gtk.Dialog):
    ...
    def set_widget_text(self):
        if self.auth_state is self.AuthState.AUTHORIZED:
            ...
            self.label.set_text(_('Elpis is Authorized with Last.fm'))

        elif self.auth_state is self.AuthState.NOT_AUTHORIZED:
            self.button.set_label(_('Authorize'))
            self.label.set_text(_('Elpis is not Authorized with Last.fm'))
