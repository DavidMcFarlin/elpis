...
class JournalLoggingPlugin(ElpisPlugin):
    ...
    def on_prepare(self):
        try:
            ...
            self._journal = JournalHandler(SYSLOG_IDENTIFIER='io.github.DavidMcFarlin.Elpis')
...
class LoggingPluginPrefsDialog(Gtk.Dialog):
    ...
    def __init__(self, parent, settings):
        ...
        self.elpis_window = parent
        self.settings = settings

        self.connect('close-request', lambda *ignore: self.response(Gtk.ResponseType.CANCEL) or True)

        sub_title = Gtk.Label.new(_('Set the journald logging level for Elpis'))
    ...
    def do_response(self, response):
        ...
        dialog = Gtk.MessageDialog(
            transient_for=self.elpis_window.prefs_dlg,
            modal=True,
