...
def _get_plugin_settings(name):
    return Gio.Settings.new_with_path('io.github.DavidMcFarlin.Elpis.plugin', '/io/github/DavidMcFarlin/Elpis/{}/'.format(name))


def maybe_migrate_settings():
    config_file = os.path.join(GLib.get_user_config_dir(), 'elpis.ini')
    prefs = {}
    try:
