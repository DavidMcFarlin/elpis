...
class ElpisPlugin(GObject.Object):
    __gtype_name__ = 'ElpisPlugin'

    _ELPIS_PLUGIN = True # used to find the plugin class in a module
    preference = None
    description = ""
...
def load_plugin(name, window, bus):
    try:
        module = __import__('elpis.plugins.' + name)
    ...
    for key, item in module.__dict__.items():
        if hasattr(item, '_ELPIS_PLUGIN') and key != "ElpisPlugin":
            plugin_class = item
            break
    else:
...
def _maybe_migrate_setting(new_setting, name):
    ...
    old_setting = Gio.Settings.new_with_path('io.github.DavidMcFarlin.Elpis.plugin', '/io/github/DavidMcFarlin/Elpis/{}/'.format(name))
    ...
    def on_got_bus(source, result, userdata):
        ...
        for name in discovered_plugins:
            ...
            if settings_name in in_tree_plugins:
                ...
                # Out of tree plugin
                plugin.settings = Gio.Settings.new_with_path('io.github.DavidMcFarlin.Elpis.plugin',
                                                         '/io/github/DavidMcFarlin/Elpis/{}/'.format(settings_name))
