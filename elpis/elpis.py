...
from . import AboutElpisDialog, PreferencesElpisDialog, SearchDialog, StationsDialog
...
@Gtk.Template(resource_path='/io/github/DavidMcFarlin/Elpis/ui/ElpisWindow.ui')
class ElpisWindow(Gtk.ApplicationWindow):
    __gtype_name__ = "ElpisWindow"
    __gsignals__ = {
    ...
    def __init__(self, app, test_mode):
        ...
        self.prefs_dlg = PreferencesElpisDialog.PreferencesElpisDialog(transient_for=self)
    ...
    def init_core(self):
        ...
        try:
            ...
            self.tempdir = os.path.join(tempdir_base, 'elpis_cache')
    ...
    def init_ui(self):
        GLib.set_application_name("Elpis")
    ...
    def set_proxy(self, *ignore, reconnect=True):
        # proxy preference is used for all Elpis HTTP traffic
    ...
    def start_song(self, song_index):
        ...
        self.set_title("%s by %s - Elpis" % (song.title, song.artist))
    ...
    def api_update_dialog(self):
        ...
        def on_response(dialog, response):
            dialog.disconnect_by_func(on_response)
            if response:
                open_browser("https://github.com/DavidMcFarlin/elpis/itbroke", self)
            self.quit()
    ...
    def on_gst_plugin_installed(self, result, userdata):
        if result == GstPbutils.InstallPluginsReturn.SUCCESS:
            self.fatal_error_dialog(_("Codec installation successful"),
                        submsg=_("The required codec was installed, please restart Elpis."))
        else:
    ...
    def react_to_buffering_message(self, from_timeout):
        ...
        # cares about the actual state, the rest of Elpis only cares about our buffer_recovery
    ...
    def on_gst_source(self, player, params):
        """ Setup httpsoupsrc to match Elpis proxy settings """
    ...
    def show_about(self, version):
        about = AboutElpisDialog.AboutElpisDialog(transient_for=self)
    ...
    def show_preferences(self):
        """preferences - display the preferences window for elpis """
    ...
    def quit(self, widget=None, data=None):
        """quit - signal handler for closing the ElpisWindow"""
        Gio.Application.get_default().quit()

    @Gtk.Template.Callback()
    def on_destroy(self, widget=None):
        """on_destroy - called when the ElpisWindow is closed. """
        self.stop()
        self.quit()
