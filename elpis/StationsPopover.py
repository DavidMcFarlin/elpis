...
class StationsPopover(Gtk.Popover):
    ...
    def __init__(self, elpis=None):
        ...
        settings = Gio.Settings.new('io.github.DavidMcFarlin.Elpis')
    ...
    def _ctx_listen(self, *_ignore):
        ...
        self.elpis.station_changed(self._ctx_station)

    def _ctx_info(self, *_ignore):
        if self._ctx_station is None:
            return
        open_browser(self._ctx_station.info_url, parent=self.elpis)

    def _ctx_seeds(self, *_ignore):
        if self._ctx_station is None:
            return
        from .StationSeedsDialog import StationSeedsDialog
        StationSeedsDialog(self.elpis, self._ctx_station).present()
    ...
    def _ctx_share(self, *_ignore):
        ...
        def on_response(dlg, response):
            if response == Gtk.ResponseType.APPLY:
                ...
                if emails:
                    self.elpis.worker_run(station.share, (emails,), None, _('Sharing station…'))
            dlg.destroy()
    ...
    def _ctx_delete(self, *_ignore):
        ...
        def on_response(dialog, response):
            ...
            if response != Gtk.ResponseType.YES:
                return
            was_current = self.elpis.current_station is station
            self.elpis.worker_run(station.delete, (), None, _('Deleting station…'))
            self.elpis.remove_station(station)
            if was_current and len(self.elpis.stations_model):
                self.elpis.station_changed(self.elpis.stations_model[0][0])
