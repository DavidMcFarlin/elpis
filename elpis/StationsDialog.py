...
@Gtk.Template(resource_path='/io/github/DavidMcFarlin/Elpis/ui/StationsDialog.ui')
class StationsDialog(Gtk.Dialog):
    ...
    def __init__(self, elpis, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.init_template()

        self.elpis = elpis
        self.model = elpis.stations_model
        self.worker_run = elpis.worker_run
...
#        self.modelsortable.set_sort_column_id(1, Gtk.SortType.ASCENDING)
    ...
    def station_renamed(self, cellrenderertext, path, new_text):
        ...
        def errorback(e):
            self.elpis.statusbar.pop(self.elpis.statusbar.get_context_id('net'))
    ...
    def on_menuitem_listen(self, widget):
        ...
        self.elpis.station_changed(station)
    ...
    def on_menuitem_seeds(self, widget):
        ...
        if station is None:
            return
        dlg = StationSeedsDialog(self.elpis, station, parent=self)
        dlg.present()
    ...
    def on_menuitem_delete(self, widget):
        ...
        def on_response(dialog, response):
            ...
            if response == Gtk.ResponseType.YES:
                ...
                self.elpis.remove_station(station)
                if self.elpis.current_station is station:
                    self.elpis.station_changed(self.model[0][0])
    ...
    def refresh_stations(self, widget):
        self.elpis.refresh_stations(self.elpis)
    ...
    def station_added(self, station, user_data):
        ...
        for existing_station in self.model:
            if existing_station[0].id == station.id:
                self.elpis.station_already_exists(existing_station[0], description, music_type, self)
                return
        ...
        self.elpis.pandora.stations.append(station)
